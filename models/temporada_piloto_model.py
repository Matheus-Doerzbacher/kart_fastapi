...
class TemporadaPilotoModel(settings.DBBaseModel):
    ...
    id_piloto: int = Column(
        Integer,
        ForeignKey("pilotos.id_piloto"),
        nullable=False,
    )
    ...
    # Relacionamentos
    piloto = relationship(
        "PilotoModel", back_populates="temporada_atual", lazy="joined"
    )
    temporada = relationship("TemporadaModel", back_populates="pilotos", lazy="joined")
